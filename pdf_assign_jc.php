<?php
    ...
    // MPESA NUMBER QUERY
    $mpesaQuery = "SELECT number, name FROM tbl_mpesa_number";
    
    $pdf_results = mysqli_query($conn, $pdf_query);
    $mpesa_results = mysqli_query($conn, $mpesaQuery);//Mpesa

    $count = mysqli_num_rows($pdf_results);
    $count2 = mysqli_num_rows($mpesa_results);//Mpesa
    ...
    if($count>0 && $count2>0){
        ...
        // MPESA
        $numbersAndNames = array();
        while ($row = mysqli_fetch_array($mpesa_results, MYSQLI_ASSOC)) {
            $numbersAndNames[] = $row["number"] . ' (' . $row["name"] . ')';
        }

        // Join the array elements into a string separated by commas
        $displayText = implode(', ', $numbersAndNames);
        ...
        $content .= 
            ...
            <tbody>
            <tr class="data">
               <td><strong>JOB CARD I: </strong>'.$pdf_data_row['id'].'</td>
            ...
            <tr class="data">
                <td colspan="4"><strong>Name:</strong> '.$pdf_data_row['customer_name'].'</td>
                
                
            </tr>

            <tr class="data">
                <td colspan="3"><strong>Address:</strong> '.$pdf_data_row['address'].',
                '.$pdf_data_row['area'].',
                <strong>'.$pdf_data_row['county'].'</strong>,
                '.$pdf_data_row['city'].'
            ...
            <tr class="data">
                <td colspan="2"><strong>Person 1:</strong> '.$pdf_data_row['contact_name_1'].' -
                    '.$pdf_data_row['contact_number_1'].'</td>
                <td><strong>Company:</strong> '.$pdf_data_row['company_name'].'</td>
            </tr>

            <tr class="data">
                <td colspan="2"><strong>Person 2:</strong> '.$pdf_data_row['contact_name_2'].' -
                 '.$pdf_data_row['contact_number_2'].'</td>
                <td><strong>Ref:</strong> '.$pdf_data_row['sales_agent'].'</td>
            </tr>
            ...
            <tr class="data">
               <td><strong style="color: #0D6EFD;">Last JC: </strong>'.$pdf_data_row['last_jc_number'].'</td>
               <td><strong style="color: #0D6EFD;">JC Date: </strong>'.$pdf_data_row['last_jc_date'].'</td>
               <td><strong style="color: #0D6EFD;">Assigned To:</strong> '.$pdf_data_row['last_assigned_to'].'</td>
            ...
            <tr class="data">
                <td colspan="4"><strong style="color: #0D6EFD;">To be filled by Technician:</strong> Work Done Date:_________ Time Started:______Time Completed:_______</td>
            ...
            <tr class="data">
               <td colspan="2">Flow of Water (500ml): <strong style="color: #0D6EFD;">Pure Water:</strong> _______Sec. </td>
               <td colspan="1"><strong style="color: #0D6EFD;">Reject Water:</strong> _______Sec. </td>
            ...
            <tr class="data">
                <td colspan="4" align="center"><strong>MPESA NUMBER: ' . $displayText . '</strong></td>
            </tr>
