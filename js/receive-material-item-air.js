...
document.getElementById("purchase_mode_select").addEventListener("change", function() {
...
if (selectedQuantity > 0 && selectedItem && INRPrice >= 0 && weight) {
    selectedDataItem.push({
        item: selectedItem,
        quantity: selectedQuantity,
        ...
        <tbody>
            ${selectedDataItem.map(data => `
                <tr>
                    ...
                    <td>${data.weight !== null ? data.weight : 'null'}</td>
                    <td>${data.date}</td>
...
$.ajax({
    type: 'POST',
    url: 'edit_receive_material_air.php',
    data: requestData,
    dataType: 'json',
