...
$item_id = isset($_POST['product_id']) ? $_POST['product_id'] : null;
...
if ($item_id !== false) {
    ...
    // Build the SQL query
    $item_query = "SELECT i.product_name, i.product_quantity, i.product_id, i.service_jc_id
                    FROM tbl_service_jc_item i
                    WHERE i.product_id = $item_id
    ...
    // Start HTML content
    $content = '<h1 align="left">STOCK MOVEMENT - PRODUCT</h1> 
                    <div class="row">
                        <div class="col">
    ...
    while ($row = mysqli_fetch_assoc($item_results)) {
        ...
        $stock_query = "SELECT stock_qty FROM tbl_product WHERE id = $item_id";
        ...
        $content .= '<tr>
                        ...
                        <td align="center" width="20%">' . $row['product_quantity'] . '</td>
                    </tr>';

        // Update the total quantity
        $total_qty_used += $row['product_quantity'];
        $item_name = $row['product_name'];
    }
...
$file_name = "ProductsStockMvmt_" . rand(0000, 9999) . ".pdf";
ob_end_clean();
