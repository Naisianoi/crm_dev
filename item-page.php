...
while ($business_row = mysqli_fetch_assoc($business_results)) {
    ...
    // Summary row per business
    echo "<tr>
            <td colspan='7'><b>Total:</b></td>
            <td style='text-align: center; color: #0000FF;'>{$businessTotalStock}</td>
...
echo '<table class="table">
        <thead class="thead-dark">
            <tr>
                ...
                <th style="text-align: center;">Total Min. Stock</th>
                <th style="text-align: center;">Total Stock Value</th>
                <th style="text-align: center;">Total Stock Qty</th>
                <th style="text-align: center;">Total Stock Cost</th>
            </tr>
        </thead>
        <tbody>';

// Reset business results for the loop
$business_results = mysqli_query($conn, "SELECT DISTINCT business FROM tbl_item");

while ($business_row = mysqli_fetch_assoc($business_results)) {
    $business = $business_row['business'];
    $businessName = $businessNames[$business] ?? $business;

    $query = "SELECT 
                COUNT(*) as total_items,
                SUM(min_stock) as total_min_stock,
                SUM(min_stock * auto_purchase_price) as total_min_stock_value,
                SUM(stock_qty) as total_stock_qty,
                SUM(stock_qty * auto_purchase_price) as total_stock_value
              FROM tbl_item
    ...
    $totalMinStock = $data['total_min_stock'];
    $totalMinStockValue = $data['total_min_stock_value'];
    $totalStockQty = $data['total_stock_qty'];
    $totalStockValue = $data['total_stock_value'];

    // Add to grand totals
    $grandTotalItems += $totalItems;
    $grandTotalMinStock += $totalMinStock;
    $grandTotalValue += $totalMinStockValue;
    $grandTotalStock += $totalStockQty;
    $grandTotalCost += $totalStockValue;

    echo "<tr>
            <td style='text-align: left;'>$businessName</td>
            <td style='text-align: center;'>" . number_format($totalItems) . "</td>
            <td style='text-align: center; color: red;'>" . number_format($totalMinStock) . "</td>
            <td class='text-end text-success'>" . number_format($totalMinStockValue) . "</td>
            <td style='text-align: center; color: #0000FF;'>" . number_format($totalStockQty) . "</td>
            <td style='text-align: right; color: red;'>" . number_format($totalStockValue) . "</td>
          </tr>";
}
...
echo "<tr>
        ...
        <td style='text-align: center;'>" . number_format($grandTotalItems) . "</td>
        ...
        <td style='text-align: right; color: red;'>" . number_format($grandTotalCost, 2) . "</td>
      </tr>";
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (urlParams.has('reload')) {
        // Trigger the business dropdown change event
        const businessSelect = document.getElementById('business_select_item');
        if (businessSelect) {
            // Force dropdown re-evaluation
            businessSelect.dispatchEvent(new Event('change'));
        }

        // Optionally remove the reload param to avoid repeated triggers
        if (!window.location.hash.includes('#reloaded')) {
            history.replaceState(null, '', window.location.pathname + '#reloaded');
            location.reload();
        }
    }
});
