        ...
        <div class="container" style="padding-top: 0px; margin-left: 0px; padding-bottom: 0px;">
            ...
            <div class="tab-content" id="search">
              ...
              <?php
                  ...
                  // Process stock summary for Products by purchase_mode
                  $stock_summary_product_freight_buffer = processStockSummary($product_results_freight_buffer);

                  // Process stock summary for Items by purchase_mode
                  $stock_summary_item_freight_buffer = processStockSummary($item_results_freight_buffer);
                  ...
                  $stock_summary_combined = [];
                    ...
                    if (!empty($stock_summary_combined)) {
                        echo '<table style="width: 70%; font-size: 13px; border-collapse: collapse;">'; // Adjust the font size as needed
                        ...
                        foreach ($stock_summary_combined as $combined_purchase_mode => $combined_purchase_mode_groups) {
                            ...
                            echo '<tr>';
                            echo '<td colspan="7"><h1><span style="color: #0d6efd;">' . $combined_purchase_mode . '</span></h1></td>';
                            echo '</tr>';

                            foreach ($combined_purchase_mode_groups as $combined_supplier_name => $combined_data) {
                                echo '<tr>';
                                echo '<td colspan="7" style="font-size: 15px;">SUPPLIER: <strong>' . $combined_supplier_name . '</strong></td>';
                                echo '</tr>';
                                ...
                                if (!empty($combined_data['products'])) {
                                    ...
                                    echo '<th style="width: 30%; text-align: center;"><strong>Product</strong></th>';
                                    echo '<th style="width: 5%; text-align: center;"><strong>Stock Qty</strong></th>';
                                    echo '<th style="width: 5%; text-align: center;"><strong>Min. Stock</strong></th>';
                                    echo '<th style="width: 7%; text-align: center;"><strong>Purchase Price</strong></th>';
                                    echo '<th style="width: 10%; text-align: center;"><strong>Auto Purchase Price</strong></th>';
                                    ...
                                    foreach ($combined_data['products'] as $row) {
                                        echo '<tr>';
                                        echo '<td style="text-align: center; width: 2%;">' . $counter++ . '.</td>';
                                        echo '<td style="width: 30%;">' . $row['name'] . '</td>';
                                        echo '<td style="text-align: center; width: 5%;">' . $row['stock_qty'] . '</td>';
                                        echo '<td style="text-align: center; width: 5%;">' . $row['min_stock'] . '</td>';
                                        echo '<td style="text-align: right; width: 7%;">' . number_format($row['price']) . '</td>';
                                        echo '<td style="text-align: right; color: green; width: 10%;">' . number_format($row['auto_purchase_price']) . '</td>';
                                        // Add more columns as needed
                                        echo '</tr>';
                                ...
                                if (!empty($combined_data['items'])) {
                                    echo '<tr>';
                                    echo '<th style="width: 2%; text-align: center;" ><strong>Sn</strong></th>';
                                    echo '<th style="width: 30%; text-align: center;"><strong>Item</strong></th>';
                                    echo '<th style="width: 5%; text-align: center;"><strong>Stock Qty</strong></th>';
                                    echo '<th style="width: 5%; text-align: center;"><strong>Min. Stock</strong></th>';
                                    echo '<th style="width: 7%; text-align: center;"><strong>Purchase Price</strong></th>';
                                    echo '<th style="width: 10%; text-align: center;"><strong>Auto Purchase Price</strong></th>';
                                    ...
                                    foreach ($combined_data['items'] as $row) {
                                        echo '<tr>';
                                        echo '<td style="text-align: center; width: 2%;">' . $counter++ . '.</td>';
                                        echo '<td style="width: 30%;">' . $row['item_name'] . '</td>';
                                        echo '<td style="text-align: center; width: 5%;">' . $row['stock_qty'] . '</td>';
                                        echo '<td style="text-align: center; width: 5%;">' . $row['min_stock'] . '</td>';
                                        echo '<td style="text-align: right; width: 7%;">' . number_format($row['price']) . '</td>';
                                        echo '<td style="text-align: right; color: green; width: 10%;">' . number_format($row['auto_purchase_price']) . '</td>';
                    ...
                    $stock_summary_combined_freight_buffer = [];

                    foreach ($stock_summary_product_freight_buffer as $purchase_mode => $supplier_groups) {
                          foreach ($supplier_groups as $supplier => $products) {
                              $stock_summary_combined_freight_buffer[$purchase_mode][$supplier]['products'] = $products;
                          }
                      }

                      foreach ($stock_summary_item_freight_buffer as $purchase_mode => $supplier_groups) {
                          foreach ($supplier_groups as $supplier => $items) {
                              if (!isset($stock_summary_combined_freight_buffer[$purchase_mode][$supplier])) {
                                  $stock_summary_combined_freight_buffer[$purchase_mode][$supplier] = [];
                              }
                              $stock_summary_combined_freight_buffer[$purchase_mode][$supplier]['items'] = $items;
                          }
                      }


                    if (!empty($stock_summary_combined_freight_buffer)) {
                      echo "<h1 style='padding-top: 0px;'>Nearly Reaching Min. Stock</h1>";
                      echo "<table style='width: 70%; font-size: 13px; border-collapse: collapse;'>";

                      foreach ($stock_summary_combined_freight_buffer as $mode => $suppliers) {
                          if (empty($mode)) continue;

                          echo "<tr><td colspan='7'><h1><span style='color: #0d6efd;'>$mode</span></h1></td></tr>";

                          foreach ($suppliers as $supplier => $data) {
                              echo "<tr><td colspan='7' style='font-size: 15px;'>SUPPLIER: <strong>" . htmlspecialchars($supplier) . "</strong></td></tr>";
