...
while ($business_row = mysqli_fetch_assoc($business_results)) {
    ...
    if (mysqli_num_rows($product_results) > 0) {
        ...
        <td class='text-center'>
                                <a href='#' class='btn btn-info edit_item_btn btn-sm' title='Edit' target='_blank'>
                                    <i class='bi bi-pen'></i>
                                </a>
                            </td>
}*/
...
$query_check_ranges = "
    SELECT 
        ...
        SUM(CASE WHEN si.work_done_date >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH) THEN si.item_quantity ELSE 0 END) AS count_1_month,
        SUM(CASE WHEN si.work_done_date >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH) THEN si.item_quantity ELSE 0 END) AS count_3_month,
        SUM(CASE WHEN si.work_done_date >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH) THEN si.item_quantity ELSE 0 END) AS count_6_month,

        SUM(CASE WHEN si.work_done_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 29 DAY) AND CURDATE() THEN si.item_quantity ELSE 0 END) AS count_1_30,
        SUM(CASE WHEN si.work_done_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 60 DAY) AND DATE_SUB(CURDATE(), INTERVAL 30 DAY) THEN si.item_quantity ELSE 0 END) AS count_31_60,
        SUM(CASE WHEN si.work_done_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 90 DAY) AND DATE_SUB(CURDATE(), INTERVAL 61 DAY) THEN si.item_quantity ELSE 0 END) AS count_61_90,
        SUM(CASE WHEN si.work_done_date BETWEEN DATE_SUB(CURDATE(), INTERVAL 120 DAY) AND DATE_SUB(CURDATE(), INTERVAL 91 DAY) THEN si.item_quantity ELSE 0 END) AS count_91_120

    FROM tbl_service_jc_item si
    WHERE si.work_done_date IS NOT NULL
    GROUP BY si.item_id
";

$result_counts = mysqli_query($conn, $query_check_ranges);
...
while ($business_row = mysqli_fetch_assoc($business_results)) {
    ...
    while ($category_row = mysqli_fetch_assoc($category_results)) {
        ...
        while ($sub_category_row = mysqli_fetch_assoc($sub_category_results)) {
            ...
            echo "<tr><td colspan='16' style='font-size:18px;'><strong style='color: #1997D4;'>$currentCategory</strong>: <span style='color: green;'>$currentSubCategory</span></td></tr>";
            ...
            if (mysqli_num_rows($item_results) > 0) {
                while ($row = mysqli_fetch_array($item_results)) {
                    ...
                    $count_1_30 = $count_data[$item_id]['count_1_30'] ?? 0;
                    $count_31_60 = $count_data[$item_id]['count_31_60'] ?? 0;
                    $count_61_90 = $count_data[$item_id]['count_61_90'] ?? 0;
                    $count_91_120 = $count_data[$item_id]['count_91_120'] ?? 0;
                    ...
                    echo "
                        <tr>
                            ...
                            <td class='text-center'>" . htmlspecialchars($row["brand"]) . "</td>
                            <td class='weight-column text-center'>" . htmlspecialchars($row["purchase_mode"]) . "</td>
                            <td class='text-center' style='color: blue;'>" . htmlspecialchars($row["stock_qty"]) . "</td>
                            <td class='weight-column text-center' style='color: red;'>" . htmlspecialchars($row["min_stock"]) . "</td>
                            ...
                            <td class='text-center'>
                                <a href='stock-view-item-date-page.php?id={$row['id']}' class='btn btn-primary btn-sm' title='Stock View'>
                                    <i class='bi bi-arrow-left-right'></i>
                                </a>
                            </td>
                            <td class='text-center'>
                                <a href='#' class='btn btn-info edit_item_btn btn-sm' title='Edit' data-id='{$row['id']}'>
                                    <i class='bi bi-pen'></i>
                                </a>
                            </td>
                        </tr>
                ...
                echo "<tr><td colspan='15' class='text-center text-muted'>No items found in $currentSubCategory</td></tr>";
            }
        }
    }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        try {
                            ...
                            // Open in a new tab
                            window.open('edit-item-page.php', '_blank');
                        } catch (e) {
